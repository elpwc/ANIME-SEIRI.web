                  ...
                  case 0:
                    ...
                    $.ajax({
                      ...
                      success: function (res_) {
                        ...
                        console.log('suc');
                        console.log(res_);
                        console.log(res_.exist);

                        if (res_.exist == 'yes') {//有屏蔽词
                          $("#register_tips-message").html('用户名里出现了奇怪的东西捏，为了网站能活下去 请不要用这些词捏❤');
                          $("#register_btn").removeAttr("disabled");
                        } else if (res_.exist == 'no') {//没屏蔽词
                          if (!verified) {//RECAPTCHA检查
                            $("#register_tips-message").text('没有通过人机身份验证捏');
                            $("#register_btn").removeAttr("disabled");
                          } else {
                            if ($('#register_u_checkbox').prop('checked')) {//用户协议检查
                              $("#register_tips-message").text('小朋友等16岁生日过了再来吧');
                              $("#register_btn").removeAttr("disabled");
                            } else {
                              if ((!$('#register_u_checkbox2').prop('checked'))) {//用户协议检查
                                $("#register_tips-message").text('ANIME-SEIRI不提供动画资源捏，请在上面确认一下w');
                                $("#register_btn").removeAttr("disabled");
                              } else {
                                //冲！
                                $.ajax({

        ...
        $.ajax({//邮箱是否已经被注册
          ...
          success: function (res) {
            ...
            if (res.exist == 'yes') {//被注册了
              ...
              $("#serify_btn").removeAttr("disabled");
              ...
              $.ajax({//发邮件
                ...
                success: function (res) {
                  ...
                  //验证码倒计时
                  max_sec = 40;
                  $("#serify_btn").html(String(max_sec) + "秒后可重发");//第一秒的显示
                  ...
                  if (!verified) {//RECAPTCHA检查
                    ...
                    if ($('#u_checkbox').prop('checked')) {//用户协议检查
                      ...
                      $.ajax({
                        ...
                        cache: false,
                        success: function (res) {
                          $("#tips-message").html('注册成功了应该, 马上进行一个刷新<br/>没反应的话点<a href="index.php">这里</a>');
...
function countdown() {
  if (max_sec > 1) {
    max_sec--;
    $("#serify_btn").html(String(max_sec) + "秒后可重发");
  } else {
    clearInterval(timer1);
    $("#serify_btn").removeAttr("disabled");
    $("#serify_btn").html("发送验证码");
  }
